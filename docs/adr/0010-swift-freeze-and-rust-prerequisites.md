...
一句话说清本决定的两半：**方向上 Swift 本体改为只修 Bug、不再加功能，本体演进交给 Rust；执行上先完成文档、方案、测试与构建等准备，再逐模块迁移。** 这是 2026-09-26 用户拍板的——「先把文档、改造方案、测试框架等前期准备工作都弄齐了再转 Rust」。M1/M2/M4/M5 是进入迁移的准备门；M3 是迁移本身的验收目标，不能要求 M3 完成后才开始 M3。
