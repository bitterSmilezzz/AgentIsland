...
| **覆盖的 agent** | **17 家**：10 家纯 `official` + 3 家混合标注（Antigravity / MiniMax / Amp 都含「otherwise / derived」）+ 2 家 `derived` + 2 家 `local runtime`（README 表逐行统计）；其中 12 家明确「借机器上已有工具的登录态」 | 25 个 agent id（Swift registry），无 usage 接入 | 面宽一个数量级 | **我们不依赖 agent 配合**：进程表 + 会话日志尾，装没装 CLI 都有状态 | **不跟进面宽**。理由同 workbench 已定项（CC Switch 10 家已占据 provider 面）；我们连 Claude Code 都不做（用户不用，Phase 2 只做 Codex） |
