...
Rust 核心 2670 行几乎零改动复用，本机配置文件实扫后第一阶段只做 Claude Code 与 Codex。
...
4. **技术栈沿用 Tauri v2 + Rust 核心**（`app/` 已跑通），Windows WPF 端不承担新形态。
...
| Phase | 内容 | 状态 |
| :--- | :--- | :--- |
| 0 | 修脱敏闸门假绿（`scan-secrets.sh`）+ 补 7 条守卫测试 | ✅ 已完成 |
| 1 | `shell_mode` 设置项 + 侧边栏壳 + 双形态并存 | 未开始 |
| 2 | CC Switch 模块（Provider 档位读写/切换） | 未开始 |
| 3 | ToDos 模块 | 未开始 |
