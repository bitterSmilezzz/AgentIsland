...
extension View {
    /// 把命中区的**高度**补到 24pt，视觉尺寸与横向占位都不变。
    ///
    /// 试过 24×24 的完整版，实测不成立：顶栏那一排（搜索 / 工具箱 / 历史 / 收起）
    /// 每个 chip 多占 6pt 宽，五个累计把整排往左推，直接盖住了状态徽标「工作中」
    /// （截图对比过）。岛内每一行右侧都排着内存/状态徽标，横向都没有余量。
    ///
    /// 高度是免费的：这些行的高度由标题文字决定（≥24pt），补高度不动任何布局。
    /// 24pt 这个数的来由：macOS 没有 iOS 那条 44pt 规矩，但指针目标低于 ~24pt 时
    /// 「hover 才显形」的小图标会明显点不中——而它们恰好都是 hover 才出现的。
    /// 只补**纵向**命中区，横向一点不让。
    /// 顶栏那一排（搜索 / 工具箱 / 历史 / 收起）实测过：改成 24×24 后每个 chip 多占 6pt 宽，
    /// 五个加起来把整排往左推，结果盖住了状态徽标「工作中」——密度换命中区在这排不划算。
    /// 行高本来就 ≥24（由标题文字决定），所以补高度是免费的。
    func hitTargetHeight(_ side: CGFloat = 24) -> some View {
        self.frame(minHeight: side)
