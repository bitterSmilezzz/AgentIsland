...
### 📉 Token 明细只留 70 天，其余折进合计——累计与「上一周期」一个数没变
...
- 做法是**折入而不是丢弃**：明细只留最近 70 天，更早的响应压成按工具的合计。
  累计口径照含这份合计，24h 与图表本来也看不到它们。另加单文件 20,000 条的硬上限，
  触顶折掉**最早**那一段。
- 70 天 = 2×最宽分析档 + 10 天余量，不是拍的。第一版按 40 天定，漏了另一半：30 天那一档
  除了画图还要往前读同样长的「上一周期」（`previousStart = now - 2 × duration`），
  而 SQLite 源本来就查得到 60 天前——JSONL 只留 40 天的话，**「较上一周期 ±N%」会被静默
  少算一块，图表上却完全看不出来**。补一条用例把窗口钉成 `≥ 2×最宽档`，动任何一边都会红。
...
- 真机对拍：改动前后 `agentisland tokens --json` 输出**逐字节相同**（累计 260,163,366、
  ...
  是 24 天前，70 天的窗口一条都够不着——真机走的是「完全没折」这条分支，折入那条分支
  由注入时钟的用例覆盖（不拿真实个人日志跑测试）。
- 明确接受的不精确：**跨文件**重复且两份拷贝时间戳不同时，70 天前的那一份会各折一次、
