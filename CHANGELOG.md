...
（release 构建、合成语料）——**每条留存明细约 1.2KB 常驻、整趟重建 0.63ms/千条**，
两者都只随磁盘语料总量线性上升；本机 24 天的日志已经是 103 份文件 / 200MB。
v0.0.9x 那道「戳备忘录」管不住它：备忘录只在整棵日志树没变时命中，**任何一次写入**
都要把整棵历史树重摊一遍。

- 做法是**折入而不是丢弃**：明细只留最近 40 天（分析页最宽 30 天 + 10 天余量），
  更早的响应压成按工具的合计。累计口径照含这份合计，24h 与图表本来也看不到它们。
  另加单文件 20,000 条的硬上限，触顶折掉**最早**那一段。
...
- 真机对拍：改动前后 `agentisland tokens --json` 输出**逐字节相同**（累计 260,163,366、
  24h 2,384,098）。窗口在今天的真机语料上一条都折不掉，所以这不是一句「大概没影响」。
- 明确接受的不精确：**跨文件**重复且两份拷贝时间戳不同时，40 天前的那一份会各折一次、
...
- 复核（差分跑改前/改后同一套写入与采样）抓到第一版一处真 bug：折入接在增量路径上时，
  去重看不见「先前已经折掉的那一段键」。后果是 **resume 一个 40 天前的老会话**时同一响应
...
- 顺手修掉一处热路径：解析时原先每行先 `Data(line)` 拷贝再找标记，等于把整段 mmap 逐行
  搬上堆；日志里 99% 以上的行是对话正文（本机 200MB 只对应 1,215 条响应），
  改成先在切片上找标记、命中才拷贝。
