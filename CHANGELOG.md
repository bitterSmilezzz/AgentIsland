...
Rust 快照加入可观测性结论和依据，按 Swift 的优先级区分可信状态、会话源读不到、
无本地明细、未接入明细源与未安装。在线 Agent 缺乏待机证据时，行内显示具体原因；
顶栏也不再把它们统称为「全部 Agent 待机」。这样缺少数据不会被误当成确认空闲。

判定只使用 Rust 目前真实拥有的证据：进程表、已登记会话根目录的可读性、近期文件写入
与历史 token 用量。没有安装探测就不产出「未安装」；深层文件/解析失败仍未纳入
`blindSessionSource`，十分钟写入窗口也不等于 Swift 的活跃会话数。这些差异在
[两端口径对照表](docs/workbench/23-swift-rust-parity-matrix.md) 明示，后续 M3 继续补证据来源。

发版扫描在当前 macOS 上因 Bash 逐文件密集启动子进程触发 `SIGTRAP`，改为每条规则
批量扫描并检查 `grep` 退出码；历史对象流固定使用 C locale，避免二进制内容引发
多字节转换失败。七条扫描器守护同步适配并通过，棘轮基线未放宽。
本轮不迁入其余缺失模块，不开放侧边栏；正式下载包仍是 Swift 原生端。
...
ADR 0010 的 M1 最后一块是 `provider.rs` 的写盘与脱敏守护。新增 Rust Provider 模块，
将临时文件创建、权限、写后验证、同目录替换与失败清理收在一个接口里；验证失败时
原文件逐字节保留，新文件也不会留下半截。Unix 临时文件使用 `0600` 权限，
目标为符号链接时拒绝写入。

Codex profile 更新路径通过 `toml_edit` 修改原生 `<name>.config.toml`：保留无关注释、
插件表、键序与多行数组；只更新 `model` / `model_provider`，后者为空时移除旧覆盖、
改为继承 base 配置。对 `auth.json` 路径直接拒绝。解析错误只返回泛化诊断，
不把可能含凭据的 TOML 原行放进错误文本。

出站预览的密钥字段使用 `MaskedSecret` 类型；`Debug`、`Display` 与 JSON 序列化
都只输出掩码。九条合成测试覆盖格式保真、密钥脱敏、替换成功、验证失败、
新文件创建与失败清理、符号链接拒绝、损坏旧档拒写与 Provider 覆盖清除。
`cargo test --locked` 现有 **53 条通过**；M1 的三项守护已齐。

这一轮只交付写盘与脱敏基础。Codex 档位列表、凭据钥匙串、备份还原、
Tauri command 与侧边栏 UI 属于后续 Phase 2，不会因测试全绿就当作可用功能。
正式下载包仍是 Swift 原生端，Rust 模块随源码发布供迁移继续使用。

## [0.0.159] - 2026-09-26

### 补齐 Rust 五态与会话回归，修复状态误判并接入发版门禁

继续跨平台桌面端的 M1 测试地基：用合成采样回放五态，用入库的 Claude/Codex/Cline
会话 fixture 验证解析。测试不依赖本机安装 Agent CLI，也不采集私人会话。

- **工作态保持无限续期**：旧实现每拍都刷新 `phase_since`，两秒一次的采样让十秒保持期
  永远到不了终点。回归测试先复现 `Working` 无法回到 `Idle`，再改为只在真实工作信号
  出现时更新锚点；同时守住长任务、时钟回拨、进程退出重启和不同 Agent 的隔离。
- **写入新鲜度使用同一拍时钟**：判定使用传入的采样时间，避免一边模拟采样一边读真实墙钟。
  60 秒边界与未来 mtime 的钳制对齐 Swift；CPU 未测到不会单独变成工作信号。
- **Codex 结构化生命周期**：普通用户消息、过程说明不作为完成标记；明确完成事件才宣布
  完成，工具调用与输入请求按调用 ID 配对收口。Cline 的 `say=command` 纳入工作信号。
- **门禁可随仓库交付**：`test-scan-secrets.sh` 从忽略名单放行，并在隔离的临时仓库里
  验证七种扫描结果，负例必须命中预期故障原因。`release.sh` 在提交前运行脱敏守护与
  `cargo test --locked`，然后执行原有 Swift 测试和打包链路。守护跑出了 Git 故障码与
  缺基线诊断被 shell 变量解析打断的问题，已修复。
- **文档同步实际进度**：工作台入口、ADR 0010 与两端对照表区分已交付的测试/工作区
  实现和待迁移能力；README 将侧边栏、配置档位、ToDos 明确为计划范围。

本轮不启动侧边栏和 Provider 功能迁移。`provider.rs` 及其原子写入/掩码测试仍缺，
M1 只完成已有引擎与会话解析的守护，不能据此宣布全部前置门通过。
正式下载包仍为 Swift 原生端；Tauri 构建用于迁移验证，Windows/Linux 真机验收另行开展。
...
答案补进 [22 号第 9 节](docs/workbench/22-remote-control-and-one-tap-setup.md)。

**跨出局域网的手机 app 只有两条腿：让家里电脑变得可达，或有个公网的人替你们传话。
中转是后者，不是唯一解。** 四种形态：

| 形态 | 要服务器吗 | 服务器看得见你的数据吗 |
|---|---|---|
| 端口映射（路由器映射出去） | 不要 | 没有第三方，但**电脑直接裸露在公网** |
| **组网直连**（Tailscale/ZeroTier 一类） | 不要（协调节点不碰数据） | **看不见**，点对点 |
| **自建中继**（已拍板走这条） | 要，你自己出 | **看得见**，流量全过我们的手 |
| 公网协调 + 打洞直连 | 要（只经手握手） | 看不见数据，但打洞失败要退中继 |

所以「一定要中转」是错的，**「一定要有公网侧参与」基本是对的**——两端多在 NAT 后面，
总得有人先帮它们认识。

### DimRemote 的形态：本机实测推断

你现在正用 DimRemote 跟我对话，所以它是最好的样本。实测两条事实：

- 二进制内 `upgradeToTunnel`、`ERR_PROXY_TUNNEL`、`Sec-WebSocket-Key/Accept`、`sec-websocket-protocol`
- **无本地 TCP 监听**（`lsof -iTCP -sTCP:LISTEN` 无命中），进程在跑

**判定：电脑主动出站长连接 + 服务端中转。** 它不监听端口，说明不是「开服务等手机连」，
而是「连出去找隧道」——这也解释了为什么你在外面照样能用（出站连接不需要端口映射），
以及**为什么它要订阅**（中转的服务器与带宽是持续成本）。

**这正是「不用 DimRemote」的真实代价**：不是省下写 app 的功夫，而是要把这部分
持续成本自己背下来。所以自建中继是**长期运营**，不是一次性开发——
服务器 + 域名 + 证书 + 带宽 + 续期 + 宕机，且必须长期活着，否则手机就失联。

### 顺序建议：先做组网形态，再迁中继

不为省钱，为**顺序风险**：组网几乎是纯客户端工作，今天就能验证「手机操作电脑」这条核心链路；
自建中继要等域名、服务器、证书、运维齐备，**链路验证会被基建阻塞**。
链路跑通后再迁中继，代价只是把「直连」换成「连中继」——
**控制面不用改**（带鉴权的入站面与通道适配器都不动）。

真要中继时也可以只做「打不通洞时的兜底路径」，平时仍走组网——这就是第四种形态的退路设计。

**核实边界如实标注**：DimRemote 的结论是实测推断，**没读它的源码也没逆向**；
它用什么协议、有没有端到端加密、服务端是否落盘均未核实。组网产品也未做选型比对。

Swift 侧无代码改动，测试基数仍为 548 条，Rust 侧 14 条。

## [0.0.156] - 2026-09-26

### 远程操控两条拍板落定：自建中继（附不同意见）、白名单+解锁可任意；并写明 ADR 0009 须随之修订

继续 [22-remote-control-and-one-tap-setup.md](docs/workbench/22-remote-control-and-one-tap-setup.md)。
用户又拍两条：**公网接入走自建中继**、**执行权限白名单默认 + 电脑侧当面解锁后可任意**。

**我(agent)对「自建中继」记录正式不同意见**，不推翻决定但备查，因为它带来的不是多花点钱，
而是**产品性质改变**，事后才知道就晚了：

1. **我们从此持有凭据**——中继要验证「这台电脑可信」就得在中继侧存设备凭据或会话凭证，
   直接撞 [ADR 0009](docs/adr/0009-credential-boundary-borrow-dont-hold.md) 的
   「不持有非本机既有登录态的密钥」。
2. **我们从此看到流量**——中继是全通路必经点，`POST /exec` 的命令与输出都过我们的服务器，
   「不转发流量」这条界守不住了。
3. **我们从此有后端要运维**——证书续期、宕机、扩容、日志留存。今天这个仓**单人开发、无 CI**，
   中继会把「发版」从「打个 zip」变成「要管在线服务」。
4. **隐私叙事要跟着改**——今天能说「数据不出你本机」，有中继后这句话不成立，
   必须换成「流量经中继但不持久化命令内容」；**能说这句话的前提是中继真的不落盘，否则是撒谎。**

**因此写入一条硬约束**：**ADR 0009 的修订与中继立项绑在一起，不许中继先上线、口径后补。**
中继也按最小形态先上——只转发、绝不落盘、凭据只做设备绑定。

**执行权限定为两档**：默认档只有预定义管理动作（切档位/重启 agent/发通知/白名单命令）；
解锁档可任意 shell，但必须**在电脑侧当面确认**（手机自己给自己解锁等于没锁）、**有超时自动回锁**、
**解锁与命令进审计日志且命令原文不落中继**。理由：纯白名单会把「也能写代码」做废
（写代码天然要跑任意命令），全程任意则手机丢了就等于 shell 丢了。

CONTEXT.md 的口径未变（仍是「鉴权硬前置 + 远程层与通道解耦」），本轮只补方案层的约束。
Swift 侧无代码改动，测试基数仍为 548 条，Rust 侧 14 条。

## [0.0.155] - 2026-09-26

### 远程操控与一键全配：把用户的新方向落成口径，并发现鉴权不必从零造

用户在手机上（DimRemote）提出五个需求：手机上远程访问并真实操作、手机上加「红糖」、
一键全配、能自动就自动能省就省、除手机 app 外还要接 QQ 与微信机器人。
新增 [`22-remote-control-and-one-tap-setup.md`](docs/workbench/22-remote-control-and-one-tap-setup.md)。

**用户拍板三项**：远程控制权限「也能下发任务和写代码」；优先级「先做一键全配与自动连上，
红糖随后」；**手机 app 自己出，不用 DimRemote（要订阅）**。

**我(agent)声明一条不让步的**：**未经鉴权的远程执行等于把这台电脑的 shell 交出去**，
所以配对码鉴权是硬前置而非可选装饰。用户授权范围越大，鉴权越要做实——
否则等于替他做了一个他不知道自己同意的决定。这不难做，但必须做。

**最重要发现：鉴权不必从零造。** `Sources/AgentIsland/LocalEventServer.swift` 早就有
`engine.selfReportTokens`（`:43-51`）这份「可信自报令牌」，macOS 14+ 也已限定回环监听
（`:63-65`），且 macOS < 14 的暴露面被显式告警而非假装安全（`:67-68`）。

因此 **「一键全配」的落点是一次性配对，不是减少配置项**：电脑侧启动时生成配对码
（复用同一个 token store，避免两处句柄「一份看得到一份看不到」），手机输一次拿令牌，
此后新设备只输配对码、新模型新账号只在电脑侧加档位、手机自动可见。这就是「一次配好」。

**架构定为两层**：一套带鉴权的入站控制面（`GET /state` / `POST /command` / `POST /exec` /
`POST /pair`）+ 可增删的通道适配器（手机 app / QQ bot / 微信 bot / 将来 Telegram、飞书）。
加第四个通道是加适配器，不是重做一遍。此前调研的 DeepChat
（[11 号](docs/workbench/11-deepchat-another-route.md):40）已用五个 IM 通道验证这条路可行，
但它因此从监控工具变成了 agent 前端——我们要守住这条界：**bot 可下发任务，
不承接「在 IM 里写代码」的 IDE 化交互**。

### QQ bot 两种模式核过官方文档：走 WebSocket 出站，不要公网 IP

[QQ 机器人事件订阅文档](https://bot.q.qq.com/wiki/develop/api-v2/dev-prepare/interface-framework/event-emit.html)
给两条路，成本差一个量级：**WebSocket 长连接是出站、不需要公网 IP/域名/端口映射**
（OpCode 10 Hello 下心跳周期 → OpCode 2 Identify 带 `token`/`intents`/`shard` → 心跳 ACK →
Resume 与重连）；**Webhook 是入站、必须公网 HTTPS 且端口只能 80/443/8080/8443**，
还要做 ed25519 签名校验（`plain_token` + `event_ts` 用 seed 派生私钥）。
走 WebSocket，与用户「能省就省」直接一致。

它跟入站控制面互补而非矛盾：QQ 这一路出站，天然不暴露本机监听面；
**真正需要鉴权硬闸门的是手机 app 那一路的入站控制面**——那才是要接到公网的一侧。

### 手机 app 那一路的公网问题，推荐组网不走中继

自家 app 要连上家里的电脑只有三种形态：直连组网 / 自建中继 / 走云函数。
**推荐组网**（文档以 Tailscale 举例，**未做选型比对**）：点对点、零服务器成本、凭据不出本机，
最贴合既有「不碰网络、不持有凭据」口径（[ADR 0009](docs/adr/0009-credential-boundary-borrow-dont-hold.md)）。
自建中继会让产品从「本机工具」变成「有后端的服务」，是另一个量级的事。**这一步要用户确认。**

**「红糖」先只定边界**：不是 agent、不是第二个对话模型，而是已有五态与 token 数据的
人格化出口；必须读同一份 `AgentObservability` 口径（否则「红糖说在跑、卡片说空闲」正好击穿
我们唯一的外部可验证差异——三个出口一致）；动效遵循克制是默认项。

**两条待再拍板**：① 组网方案（决定要不要买域名与服务器）；② `POST /exec` 是任意 shell
还是白名单管理动作——这是把 shell 交出去的范围，由你(用户)定。

CONTEXT.md 同步两条硬口径（鉴权硬前置、远程层与通道解耦）。Swift 侧无代码改动，
测试基数仍为 548 条，Rust 侧 14 条。

## [0.0.154] - 2026-09-26

### 补上产品口径：这是什么产品、边界在哪，以及一个此前没对用户说过的实况

第 20 号做的是工程收敛，缺的是「这个产品是什么」。本文补上它，新增
[`21-product-positioning.md`](docs/workbench/21-product-positioning.md)，并把它设为
`docs/workbench/README.md` 的第一入口（产品定位的权威从 07 号移到 21 号）。

**定位（用户 2026-09-26 拍板）**：**一站式配置与同质、多 agent 管理、ToDos**，CC Switch 式能力收进来。
三个名词撑起它，缺一不可：

| 名词 | 今天的意思 | 为什么必须 |
|---|---|---|
| 一站式配置 | 档位切换（Phase 2，含 CC Switch 的能力） | 单点是「装一个就够」，不是「再多一个」 |
| 同质 | 26 家同一套五态、同一套口径、同一套「读不到」记号 | 没有同质，「跨」只是并列，用户要自己比 |
| 多 agent 管理 | 配置/待办/状态跨 agent 一起看一起管 | 从「监控」变成「管理」的分界 |

### 一个必须先裁的冲突：CC Switch 式能力与「同质」是相反方向

用户同时要「CC Switch 功能」和「同质」。这两个诉求物理上冲突，不能同时最大化——
CC Switch 的本质是**尊重差异**（90+ 预设供应商一个一个适配出来），
「同质」的本质是**抹平差异**。

**裁决：同质只做用户看得见的三样，其余一律尊重差异。** 同质做在**呈现层**（五态、可信度、
「读不到」记号、档位卡）才能同时保住两件事；做在存储/生效层就会变成 CC Switch 的复制品，
而那一层正是它积累最深的地方。对应到 Phase 2 就是：档位文件用 Codex 原生 `--profile`，
不自己造格式；预设供应商 0 个。

### 一个此前没对用户说过的实况：26 家里只有 5 家读得到 token

逐家读 `AgentRegistry.swift` 的 `tokenRoots` 得出：**26 家档案里 5 家非空**
（dim / claude / codex / workbuddy / workbuddy-ai），**21 家为空或未声明**
...
这不是 bug 是今天的事实，但它直接说明：**「token 成本」这个卖点今天只对 5 家成立。**
首层若按 26 家一视同仁展示，21 家会在「花了多少 token」那一栏撒谎或空缺。
用户已拍板「维持清单但把读不到显式标出」，而这正好有现成地基（`AgentObservability` 五类结论
+ CONTEXT.md 的「没查到 ≠ 零」记号），不是从零做。

**README 如实改了**：首段从「列 16 家 agent 的品类描述」改成「跨 Agent 本机管理工作台」+
一句能力边界——26 家都能报五态、5 家有 token 明细、读不到写「读不到」不写 0。
26 家完整清单移入「能看见什么」节，保留可检索性。

### 侧边栏分工定了：主攻「档位与此刻状态」，不做数据报表搬家

理由：报表类能力（趋势图、导出）今天在灵动岛里已能放下，做进侧边栏是搬家不是增量；
而「档位卡上显示此刻跑什么」是 **CC Switch 结构上给不了的**（它不读会话尾、不做五态），
是唯一真实增量。

### 不做的边界一次性列清（六条，每条带代价）

自己当 agent、架 LLM 网关/转发流量、ML 模型路由、90+ 预设供应商、
配额百分比直读（deferred 非 decided-against，需先改 ADR 0009）、
Swift/Rust parity 测试（改口径对照表说清，见 ADR 0010 M5）。
