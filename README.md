...
### 「人走开了才提醒」：在场判定补上无输入时长 (v0.0.101)
- v0.0.100 的判定只认「屏幕锁定」与「显示器睡眠」。对**远程桌面连着 Mac** 的日常用法这两条永远不成立：会话不锁、屏幕不熄，人早走了——实测就是这台机器的形态（无 `CGSSessionScreenIsLocked` 键、显示器未睡、键盘 771 秒没动）。结果那个开关一开就等于永远不发。
- 现在加第三条「无输入超过 N 秒」，默认 120 秒，`CGEventSource.secondsSinceLastEventType(.combinedSessionState)` 取键盘/左右键/移动/滚轮里最近的一次。这条正是远程场景唯一有效的信号。
- 开关效果当场可见：设置页实时显示当前判定与三条信号的值；被挡下的通知在「最近外发」里带依据（多少秒 vs 阈值）。取不到时长时按「已离开」发出去（fail-open）并把「这条判据没数据」写在页面上，不假装判定过。
### 可选的远程通知：人在 Windows 上，通知也能到手机/邮箱 (v0.0.100 / v0.0.101 / v0.0.102 / v0.0.103)
...
- 有个「只在人不在机器前时发送」的开关（默认关闭）。判定用三条信号，任一成立即算离开：屏幕锁定、显示器睡眠、**键盘鼠标无输入超过阈值**（默认 120 秒，可调 30–3600）。
  远程桌面连着 Mac 时前两条永远不会成立，**只有无输入这条管用**——你去忙别的、RDP 窗口开着但不动键鼠，就会被判成离开并收到通知。
  设置页会实时显示当前判定（例：「当前判定：已离开（距上次输入 214 秒；锁屏 否、显示器睡眠 否）」），被挡下时「最近外发」写明依据：「未发：有人在机器前（距上次输入 8 秒，未达 120 秒）」。
  取不到输入时长时会按「已离开」照常发（fail-open），并在页面上说破「这台机器上这条判据没有数据」。
- **自动外发遇到暂时性失败会隔 5 秒再试一次**（v0.0.102）。人不在机器前时，一次网络抖动就等于这条提醒永久丢失。「发送测试」不重试——它给的就是这一次的真实结果。「最近外发」会写明「已送达（重试 1 次后）」或「重试 1 次仍未送达」，通道在抖你看得见。
- **但「对方明确拒绝」不重试**（v0.0.103）：主题不存在（404）、授权码错误（SMTP 535）、拒绝中继（550）——再要一次也是同一个结果，而 ntfy.sh 这类公开中转按条数限流，白重试只会让你更快撞上上限。这种失败在页面上写的是「对端明确拒绝，重试无用」，不会伪装成「链路在抖」。
