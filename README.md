...
### 可选的远程通知：人在 Windows 上，通知也能到手机/邮箱 (v0.0.100 / v0.0.101 / v0.0.102)
...
- **自动外发失败后隔 5 秒再试一次**（v0.0.102）。人不在机器前时，一次网络抖动就等于这条提醒永久丢失。只有真的送出失败才重试——被开关、静默、在场判定、节流挡下的不重试；「发送测试」也不重试，它给的就是这一次的真实结果。「最近外发」会写明「已送达（重试 1 次后）」或「重试 1 次仍未送达」，通道在抖你看得见。
